...
\tipbox{En théorie des probabilités, notre modèle d'hétérogénéité est
  un modèle dit «urne d'urne» en ce sens que les montants de sinistres
...
\tipbox{Puisque
  ...
  \end{equation*}
  l'ordre des sinistres ne joue aucun rôle dans les calculs.}

\warningbox{Dans la suite, nous allons ne considérer qu'un seul
  contrat et laisser tomber l'indice $i$ dans la notation. En effet,
  dans l'approche entièrement paramétrique adoptée dans ce chapitre,
...
\begin{exemple}
  ...
  variable aléatoire avec distribution bêta de paramètres $\alpha$ et
  $\beta$. Ainsi,
  \begin{equation*}
  ...
  \item Nous souhaitons maintenant calculer la prime bayésienne après
    ...
    paramètres $\tilde{\alpha} = \alpha + \sum_{t = 1}^n x_t$ et
...
\begin{equation*}
  B_{i, n + 1} = \esp{\mu(\Theta)|x_1, \dots, x_n}
