...
\begin{objectifs}
\item Expliquer le modèle d'hétérogénéité de Bühlmann, notamment le
  rôle de la variable aléatoire représentant le niveau de risque.
\item Définir la prime de risque, la prime collective, la prime
  bayésienne et la prime de crédibilité.
\item Calculer la distribution à postériori pour des modèles discrets
  et continus.
\item Calculer la distribution prédictive pour des modèles discrets et
  continus.
\item Utiliser le lien entre la distribution prédictive, la
  distribution marginale et la distribution à postériori.
\item Calculer la prime bayésienne pour des modèles discrets et
  continus, en particulier pour les combinaisons de distributions
  débouchant sur une prime linéaire.
\item Énoncer les conditions du modèle de Jewell pour obtenir une
  prime bayésienne linéaire.
\item Utiliser la fonction \code{cm} du paquetage R \pkg{actuar} pour
  calculer des primes bayésiennes linéaires et interpréter les
...
portefeuille devient incontournable lorsque nous devons estimer
...
nous démontrerons que le calcul de la prime bayésienne peut parfois
...
groupe de l'hétérogénéité dans les niveaux de risque. C'est cette
...
\label{tip:baysienne:predictive}
\tipbox{La conséquence de la remarque précédente, c'est que si la
  distribution à posteriori de $\Theta$ est du même type que la
  distribution à priori, alors la distribution prédictive est du même
  type que la distribution marginale.}
  ...
  \begin{sol}
    ...
    \item Nous cherchons $\esp{\mu(\Theta)|S_1 + S_2 + S_3 = 8}$. Or,
      \begin{align*}
        ...
        \theta^{2 - 1} e^{-\theta} \\
        &= \theta^{\sum_{t = 1}^3 x_t + 2 - 1} e^{-2 \theta},
      \end{align*}
      d'où
  ...
  \begin{sol}
    ...
    \item Supposons, sans perte de généralité, que le domaine de
      ...
      \begin{align*}
        ...
        x_0 \int_{-\infty}^\infty u(\theta)\, d\theta \\
