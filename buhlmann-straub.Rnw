...
opn attribue la même crédibilité aux deux entreprises, même si leur
...
Avec l'introduction des poids dans notre modèle de crédibilité, nous
devrons dorénavant calculer des moyennes pondérées plutôt que des
moyennes arithmétiques. Nous aurons recours à la notation suivante:
\begin{align*}
  X_{iw}
...
\begin{align*}
  ...
  \hat{a}   &= \nombre{72310},
\end{align*}
ce qui mène aux résultats du \autoref{tab:buhlmann-straub:buhlman}. Le
...
\begin{exercice}
  ...
  préciser d'utiliser $X_{ww}$ comme estimateur de la prime collective
...
\begin{exercice}
  ...
  \begin{sol}
    ...
    \item Il faut utiliser la formule de $\hat{s}^2$ pour données
      manquantes. Tout d'abord, nous avons
      \begin{align*}
        w_{1\pt} &= 3  & X_{1w} &= 4 \\
        w_{2\pt} &= 10 & X_{2w} &= 8 \\
