...
\begin{exemple}[Cas Bernoulli/bêta]
...
Nous effectuerons les calculs de crédibilité avec la fonction
\code{cm} du paquetage \pkg{actuar} \citep{actuar}. Disponible dans
...
La fonction \code{cm} de \pkg{actuar} sert d'interface unique pour la
l'ajustement de plusieurs modèles de crédibilité: les modèles
bayésiens linéaires de la section précédente, les modèles de
...
négative/bêta, ainsi que le cas quelque peu périphérique Pareto
...
\begin{exercice}
  ...
  \begin{sol}
    ...
    \item L'estimateur à priori de $\theta$ (ou la «prime collective») est
      ...
      facteur $\theta$ dépend des observations et il se trouve pas
